...
select first_name,department_id,count(*) from employees
...
select manager_id,concat(first_name,' ',last_name)as Name,department_id,salary  from employees
...
-- 1. Write a query that displays the employee's last names only from the string's 2-5th position with the first letter capitalized and all other letters lowercase, Give each column an appropriate label.     
     select CONCAT(UPPER(SUBSTRING(last_name,2,1)),LOWER(SUBSTRING(last_name,3,5)))
     from employees;
...
-- 4. Display the employee ID by Appending two zeros after 2nd digit and 'A' in the end, department id, salary and the employee name all in Upper case, if the employee name consists of 'z' replace it with '$!   
       SELECT CONCAT(LEFT(employee_id, 2), '00', RIGHT(employee_id, LENGTH(employee_id) - 2), 'E') AS new_emp_id,department_id,salary,
...
-- 9.Extract first letter from First Name column and append it with the Last Name. Also add "@systechusa.com" at the end. Name the column as e-mail address.All characters should be in lower case.  Display this along with their First Name.   
       select CONCAT(lower(LEFT(first_name, 1)),lower(last_name),'@systechusa.com') as email_address,first_name 
       from employees;

-- 10. Display the name of the employee whose salary is the lowest.   
      SELECT first_name FROM employees 
      Where salary = (SELECT MIN(salary) FROM employees);
      
-- 11. Display the names of all employees except the lowest paid.
      SELECT first_name FROM employees 
      Where salary != (SELECT MIN(salary) FROM employees);
...
-- 6. Create a query that displays employees last name,department number,and all the employees who work in the same department as a given employee.   
	SELECT department_id,last_name,count(*) from employees
    group by department_id,last_name;
    
-- 7. Create a query that displays the name,job,department name,salary,grade for all employees. Derive grade based on salary(>=50000=A, >=30000=B,<30000=C)   
      alter table employees
      add column grade varchar(10);
      
      select e.first_name,e.job_id,d.department_name,e.salary,e.grade
      from employees e join departments d
	  where case
...
-- 5. Display the department number, last name, and job ID for every employee in the OPERATIONS department.   
...
-- 6. Modify the above query to display the employee numbers, last names, and salaries of all employees who earn more than the average salary and who work in a department with any employee with a 'u'in their name.   
...
-- 9. Write a query to display the top three earners in the EMPLOYEES table. Display their last names and salaries.   
      select last_name,salary from employees
      order by salary desc
	  limit 3;
      
-- 10. Display the names of all employees with their salary and commission earned. Employees with a null commission should have O in the commission column   
